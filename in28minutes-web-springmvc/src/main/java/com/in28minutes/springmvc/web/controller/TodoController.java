...
public class TodoController extends AbstractController {
    ...
    public ModelAndView loadTodoList() {
        ...
        TodoItemList todoList = todoService.getTodoListByUser(
    ...
    public String showCreateTodo(Model model) {
        model.addAttribute(
                "today", 
                new SimpleDateFormat(TodoListUtils.DATE_FORMAT).format(new Date())
        );
    ...
    public String searchTodo(@RequestParam String title, Model model) {
        TodoItemList todoList = todoService.searchTodoListByTitle(
                sessionData.getUser().getId(), 
                title
        );
